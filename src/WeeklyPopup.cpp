...
WeeklyPopup* WeeklyPopup::create(tsl::List* list) {
    WeeklyPopup* ret = new WeeklyPopup();
    if (ret->initAnchored(420, 280, list)) {
        ret->autorelease();
        return ret;
    }
...
bool WeeklyPopup::setup(tsl::List* list) {
    m_list = list;
    ...
    geode::createQuickPopup(
        ...
        [&](auto, bool btn2) {
            if (btn2) {
                ...
                m_list->setLocalWeekly(m_list->getCurrentWeekly());
                m_list->setCachedWeekly(nullptr);
...
void WeeklyPopup::showLevel(GJGameLevel* level) {
    ...
    m_discardButton->setVisible(m_list->getCurrentWeekly() != m_list->getLocalWeekly());
    m_cellContainer->setVisible(true);
}

void WeeklyPopup::loadLevel() {
    if (GJGameLevel* level = m_list->getCachedWeekly())
        showLevel(level);
    else if (m_list->getLocalWeekly() == 0)
        tsl::Request::loadWeekly(true, m_list);
    else {
        GameLevelManager* glm = GameLevelManager::sharedState();
        glm->m_levelManagerDelegate = this;
        glm->getOnlineLevels(GJSearchObject::create(SearchType::Type19, std::to_string(m_list->getLocalWeekly())));
    }
}
...
void WeeklyPopup::updateTime() {
    ...
    m_timeLabel->setString(fmt::format("New Weekly Challenge in: {}", m_list->getRemainingTime()).c_str());
}
