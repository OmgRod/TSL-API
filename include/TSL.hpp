...
namespace tsl {
    ...
    private:
        ListRegistry() = delete;
        ListRegistry(const ListRegistry&) = delete;
        ListRegistry& operator=(const ListRegistry&) = delete;
        
    public:
        static std::vector<List*>& getRegisteredLists() {
            static std::vector<List*> registry;
            return registry;
        }
        static void registerList(List* list) {
            getRegisteredLists().push_back(list);
        }
        static List* getListByID(const std::string& id) {
            auto& lists = getRegisteredLists();
            for (auto* list : lists) {
                if (list && list->m_settings && list->m_settings->listID == id) {
                    return list;
                }
            }
            
            auto* settings = new ListSettings();
            settings->name = "Unknown";
            settings->listID = id;
            settings->endpoint = "Unknown";
            settings->mod = nullptr;
            settings->levelsPerPage = 10;
            settings->weekly = false;
            settings->staffInfo = false;
            settings->staff = StaffTeam::create();
            settings->icon = nullptr;
            settings->iconSmall = nullptr;
            
            return List::create(settings);
        }
    };
